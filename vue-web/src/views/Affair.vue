  ...
  <div id="AffairPage">
    ...
    <!-- <div class="FinishBlock" v-if="RecordShowDate !=null && RecordShowDate.date != undefined">
      <p class="FinishDay">
        <span
          class="text"
        >{{Tool.getDateString(RecordShowDate.data)}}号&nbsp; 周{{Tool.FormatDateWeekChinese(RecordShowDate.date.getDay())}}</span>
        ...
        >
          <span
            ...
            :class="{HistoryDateNumber:true,
              ...
              active:Tool.getDateString(RecordShowDate.data) == Tool.getDateString(item.data)}"
            @click="RecordShowDate = HistoryRecord[i],Record.activeIndex = i"
          >
    ...
    <div class="FinishRecordPush" v-if="onPushRecord && pushAffair!=null">
      <div class="PushAlert">
        <p class="PushRecordName">
          ...
          <span
            class="AffairLevel"
            v-show="pushAffair.record.length > 0"
          >Lv.{{pushAffair.record.length}}</span>
        </p>
    ...
    keyDown() {
      document.onkeydown = e => {
        ...
        if (
          e1 &&
          e1.keyCode == 37 &&
          this.Record.activeIndex < this.HistoryRecord.length - 1
        ) {
          if (
            this.HistoryRecord[this.Record.activeIndex + 1].record.length > 0
          ) {
